...
export abstract class OfflineService<T extends BaseModel> {
  ...
  private init(): void {
    ...
    this.getUpdatesFromStorage().then((updates: Update<T>[]) => {
      this.synchronize();
    });
  ...
  private synchronize(): void {
    this.syncPushingToServer().then((updates: Update<T>[]) => {
      this.syncPullingFromServer();
    });
  }

  private startNetworkListening(): void {
    this.networkOnConnectSubscription = this.network
      .onConnect()
      .subscribe(() => {
    ...
    this.networkOnDisconnectSubscription = this.network
      .onDisconnect()
      .subscribe(() => {
  ...
  private getAllFromStorage(): Promise<T[]> {
    return this.storage.ready().then((localForage: LocalForage) => {
      ...
      return this.storage
        .forEach((value: any, key: string, iterationNumber: number) => {
          if (
            key.indexOf(`${this.resourceName}.`) > -1 &&
            key.indexOf('updates.') == -1
          ) {
            items.push(value);
          }
        })
  ...
  private deleteFromStorage(item: T): Promise<boolean> {
    return this.storage
      .remove(`${this.resourceName}.${item.id}`)
      .then(() => true);
  }
  ...
  private addUpdate(update: Update<T>): Promise<Update<T>> {
    return this.storage
      .set(`updates.${this.resourceName}.${update.value.id}`, update)
      .then((up: Update<T>) => {
  ...
  private removeUpdate(update: Update<T>): Promise<void> {
    return this.storage
      .remove(`updates.${this.resourceName}.${update.value.id}`)
      .then(() => {
  ...
  private getItemsFromCache(): Promise<void> {
    if ('caches' in window) {
      return self.caches
        .match(`${this.itemApiURL}/${this.resourceName}`) // http://localhost:3000/api/v1/tasks
        .then(response => {
          if (response) {
            return response.json().then(cachedJson => {
  ...
  private saveInServer(update: Update<T>): Observable<any> {
    ...
    switch (update.method) {
      case 'put':
        url += `/${update.value.id}`;
      case 'post':
        responseObservable = this.http[update.method](
          url,
  ...
  protected createInServer(item: T): Promise<T> {
    return this.saveInStorage(item).then((item: T) => {
      this.addUpdate(new Update<T>('post', item)).then((up: Update<T>) => {
  ...
  protected deleteInServer(item: T): Promise<void> {
    return this.deleteFromStorage(item).then((deleted: boolean) => {
      this.addUpdate(new Update<T>('delete', item)).then((up: Update<T>) => {
        let items: T[] = this.listItems$.getValue();
  ...
  private syncPullingFromServer(): Promise<T[]> {
    return this.http
      .get(`${this.itemApiURL}/${this.resourceName}`)
      .map((response: Response) => response.json())
      .toPromise()
      .then((serverData: any) => {
        if (serverData.timestamp > this.lastUpdate) {
          this.setLastUpdate(serverData.timestamp);
          return this.saveAllInStorage(serverData.data).then((items: T[]) => {
            this.listItems$.next(items);
            // clean storage
  ...
  private cleanStorage(serverItems: T[]): void {
    if (this.lastUpdate > 0) {
      this.getAllFromStorage().then((storageItems: T[]) => {
        let itemsToDelete: T[] = storageItems.filter((storageItem: T) => {
          return !serverItems.some(
            (serverItem: T) => serverItem.id === storageItem.id,
          );
        });
