...
export class MovieService {
      ...
      return this.sqliteHelperService
        ...
        .then((db: SQLiteObject) => {
          ...
          this.db
            ...
            .then(success =>
              console.log('Movie table created successfully!', success),
            )
            .catch((error: Error) =>
  ...
  getAll(orderBy?: string): Promise<Movie[]> {
    return this.getDb().then((db: SQLiteObject) => {
      return <Promise<Movie[]>>this.db
        .executeSql(`SELECT * FROM movie ORDER BY id ${orderBy || 'DESC'}`, {})
        .then(resultSet => {
          let list: Movie[] = [];
        ...
        .catch((error: Error) =>
          console.log('Error executing method getAll!', error),
        );
    });
  }
  ...
  create(movie: Movie): Promise<any> {
    return this.db
      .executeSql('INSERT INTO movie (title) VALUES (?)', [movie.title])
      .then(resultSet => {
  ...
  update(movie: Movie): Promise<any> {
    return this.db
      .executeSql('UPDATE movie SET title=? WHERE id=?', [
        movie.title,
        movie.id,
      ])
  ...
  delete(id: number): Promise<any> {
    return this.db
      .executeSql('DELETE FROM movie WHERE id=?', [id])
      .then(resultSet => resultSet.rowsAffected > 0)
      .catch((error: Error) =>
  ...
  getById(id: number): Promise<Movie> {
    return this.db
      .executeSql('SELECT * FROM movie WHERE id=?', [id])
