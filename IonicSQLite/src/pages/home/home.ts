...
export class HomePage {
  ...
  ionViewDidLoad() {
    this.movieService.getAll().then((movies: Movie[]) => {
      this.movies = movies;
    });
  }
        ...
        buttons: [
          {
            ...
            handler: () => {
              ...
              this.movieService.delete(movie.id).then((deleted: boolean) => {
                if (deleted) {
      ...
      buttons: [
        'Cancel',
        {
          ...
          handler: data => {
            ...
            this.movieService[options.type](contextMovie).then(
              (result: any) => {
